...
class Flock {
  ...
  public void createBassBoid(Metronome m, int note, float timing, float velocity) {  //bass boid
  ...
  public void createChirpBoid(Metronome m, int oct, float timing, float velocity) { //chirp boid
    Boid b = new Boid(m, random(width), random(height), timing);
  ...
  void setBoidValues(int bType, int bDegree, float bVolume) {
    if (boids.size() > 0) {
      int index = (int) random(0, boids.size());
      (boids.get(index)).setValues(bType, bDegree, bVolume); //manually set boid values
    }
  }
  
  void removeLastBoid() {
    if (boids.size() > 0) {
      boids.remove(boids.size() - 1); //Removes last boid
    }
  }
